...
export const BillingHistory = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [patientRes, billRes] = await Promise.all([
        OperatorService.getPatientById(patientId),
        OperatorService.getBillsByPatient(patientId)
      ]);
      
      setPatient(patientRes.data);
      setBills(billRes.data || []);

      const total = billRes.data.reduce((s, b) => s + (b.amount || 0), 0);
      const paid = billRes.data.filter(b => b.payment_status === 'Cleared').reduce((s, b) => s + (b.amount || 0), 0);
      const pending = billRes.data.filter(b => b.payment_status === 'Pending').reduce((s, b) => s + (b.amount || 0), 0);
      const overdue = billRes.data.filter(b => b.payment_status === 'Overdue').reduce((s, b) => s + (b.amount || 0), 0);
      setSummary({ total, paid, pending, overdue });
      
    } catch (err) {
      console.error('Failed to fetch data:', err);
      toast.error('Failed to load patient billing history');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredBills = bills.filter(b => {
    ...
    if (tabValue === 3) return b.payment_status === 'Overdue';
    return true;
  });
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 }, minHeight: '100vh' }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                  ...
                  <Box>
                    ...
                    <Stack direction="row" spacing={2} sx={{ mt: 1 }} flexWrap="wrap">
                      <Chip icon={<PhoneIcon />} label={patient?.phone || 'N/A'} size="small" />
                      <Chip icon={<EmailIcon />} label={patient?.email || 'N/A'} size="small" />
                      <Chip icon={<CalendarIcon />} label={`Patient since ${formatDate(patient?.date_joined)}`} size="small" />
                    </Stack>
                  </Box>
                </Box>
              </Grid>
      ...
      <Grid container spacing={2} sx={{ mb: 4 }}>
        {[
          { label: 'Total Billed', value: summary.total, color: theme.palette.primary.main, icon: <MoneyIcon /> },
          { label: 'Paid', value: summary.paid, color: theme.palette.success.main, icon: <CheckIcon /> },
          { label: 'Pending', value: summary.pending, color: theme.palette.warning.main, icon: <ScheduleIcon /> },
          { label: 'Overdue', value: summary.overdue, color: theme.palette.error.main, icon: <WarningIcon /> },
        ].map((stat, idx) => (
